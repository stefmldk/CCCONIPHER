    ...
    cat('\nSaving all treebuilding output\n')
{
  ...
  write.table(Reduce(rbind, strsplit(sample_pyclone_tree$
                                       graph_pyclone$
                                       consensus_branches, split = ":")), file = consensusBranchesFile, row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
  ...
  write.table(Reduce(rbind, strsplit(sample_pyclone_tree$
                                       graph_pyclone$
                                       consensus_relationships, split = ":")), file = consensusRelationshipsFile, row.names = FALSE, col.names = FALSE, quote = FALSE, sep = "\t")
  ...
  clusterInfoDF <- clusterInfoDF %>% dplyr::full_join(data.frame(sample_pyclone_tree$
                                                                   nested_pyclone$
                                                                   ccf_cluster_table, stringsAsFactors = FALSE) %>%
                                                        dplyr::mutate(clusterID = rownames(.)) %>%
                                                        tidyr::pivot_longer(!clusterID, names_to = "Region", values_to = "meanCCF"), by = c("clusterID"))
  clusterInfoDF <- clusterInfoDF %>% dplyr::full_join(data.frame(sample_pyclone_tree$nested_pyclone$ccf_ci_lower, stringsAsFactors = FALSE) %>%
                                                        dplyr::mutate(clusterID = rownames(.)) %>%
                                                        tidyr::pivot_longer(!clusterID, names_to = "Region", values_to = "CCF_CI_low"), by = c("clusterID", "Region"))
  clusterInfoDF <- clusterInfoDF %>% dplyr::full_join(data.frame(sample_pyclone_tree$nested_pyclone$ccf_ci_upper, stringsAsFactors = FALSE) %>%
                                                        dplyr::mutate(clusterID = rownames(.)) %>%
                                                        tidyr::pivot_longer(!clusterID, names_to = "Region", values_to = "CCF_CI_high"), by = c("clusterID", "Region"))
  clusterInfoDF <- clusterInfoDF %>% dplyr::full_join(data.frame(sample_pyclone_tree$
                                                                   clonality_out$
                                                                   clonality_table_corrected, stringsAsFactors = FALSE) %>%
                                                        dplyr::mutate(clusterID = rownames(.)) %>%
                                                        tidyr::pivot_longer(!clusterID, names_to = "Region", values_to = "clonality"), by = c("clusterID", "Region"))
  clusterInfoDF <- clusterInfoDF %>% dplyr::full_join(data.frame(sample_pyclone_tree$
                                                                   clone_proportion_out$
                                                                   clone_proportion_table, stringsAsFactors = FALSE) %>%
                                                        dplyr::mutate(clusterID = rownames(.)) %>%
                                                        tidyr::pivot_longer(!clusterID, names_to = "Region", values_to = "clone_proportions_default"), by = c("clusterID", "Region"))
  ...
  cp_min_sce_trees <- sample_pyclone_tree$
    clone_proportion_out$
  ...
  ses_min_sce_trees <- sample_pyclone_tree$
    subclonal_expansion_score_out$
    subclonal_exp_score_min_sce_trees
  ...
  altTreeInfoDF$sum_condition_error <- sapply(altTreeInfoDF$treeID, function(i) sample_pyclone_tree$
    graph_pyclone$
    alt_trees_sum_condition_error[i])
  ...
  altTreeInfoDF$edge_probability_score <- sapply(altTreeInfoDF$treeID, function(i) sample_pyclone_tree$
    graph_pyclone$
    alt_trees_edge_probability[i])
...
treebuilding_plot <- function(sample_pyclone_tree) {
  ...
  correct_cpn_clusters <- sample_pyclone_tree$
    parameters$
    correct_cpn_clusters
  adjust_noisy_clusters <- sample_pyclone_tree$
    parameters$
    adjust_noisy_clusters
  adjust_noisy_clusters_prop <- sample_pyclone_tree$
    parameters$
    adjust_noisy_clusters_prop
  ...
  clonality_table <- sample_pyclone_tree$
    clonality_out$
  ...
  pdf(pdfname, width = 22 * width.mult.factor, height = 12 * height.mult.factor)
{
  ...
  pie.size <- ncol(sample_pyclone_tree$
                     nested_pyclone$
                     ccf_cluster_table)
  ...
  pie.colors <- sample_pyclone_tree$
    nested_pyclone$
    ccf_cluster_table[match(names(vcol), rownames(sample_pyclone_tree$
                                                    nested_pyclone$
                                                    ccf_cluster_table)), , drop = F]
