...
clustering_preprocess <- function(input_table, new.dir, subclonal_copy_correction = TRUE, multiple_test_correction = TRUE, only_truncal_subclonal_copy_correction = TRUE, fix_absentCCFs = TRUE) {
  ...
  if (fix_absentCCFs) {
    ...
    for (region in regions.to.use) {
      tmp.clusterPresence <- clusterPresence.df %>%
        dplyr::filter(regions %in% region) %>%
...
clustering_postprocess <- function(input_list, sample.results, new.dir, input_tsv, input_seg_tsv_loc = NULL, min_cluster_size = 5, driver_cat = "1", clean_clusters = TRUE, min_ccf_present = 0.1, clonal_cutOff = 0.9, propClonal_threshold = 0.25) {
  ...
  sig.pvals <- mut.pvals[which(as.numeric(mut.pvals[, 1]) < 0.05 &
                                 as.numeric(mut.pvals[, 2]) <= 0.75 &
                                 as.numeric(mut.pvals[, 3]) == 0), 1]
  sig.table <- mut.pvals[which(as.numeric(mut.pvals[, 1]) < 0.05 &
                                 as.numeric(mut.pvals[, 2]) <= 0.75 &
                                 as.numeric(mut.pvals[, 3]) == 0), , drop = FALSE]
  ...
  output_tsv <- output_tsv %>%
    ...
    dplyr::left_join(input_tsv %>%
                       dplyr::select(CASE_ID, SAMPLE, CHR, POS, REF, ALT, DEPTH, ACF, PLOIDY) %>%
                       mutate(CHR = as.numeric(CHR), POS = as.numeric(POS)), by = c("CHR", "POS", "REF", "ALT", "SAMPLE")) %>%
  ...
  output_tsv_clean <- output_tsv %>%
    dplyr::filter(CLEAN) %>%
    dplyr::select(-CLEAN, -phyloCCF.0.05, -phyloCCF.0.95)
  output_tsv_dirty <- output_tsv %>%
    dplyr::filter(!CLEAN) %>%
    dplyr::select(-CLEAN, -phyloCCF.0.05, -phyloCCF.0.95)
