create.subclonal.copy.number <- function(seg.mat.copy, min.subclonal = 0.1) {
  ...
  # which ones work?
  seg.sorted <- seg.out[(seg.out$fracMaj1 == seg.out$fracMin1 | seg.out$fracMaj1 == seg.out$fracMin2) |
                          (seg.out$fracMaj1 == 1) |
                          (seg.out$fracMaj1 == 0) |
                          (seg.out$fracMin1 == 1) |
                          (seg.out$fracMin1 == 0), , drop = FALSE]
                                ...
                                , minVAFtoEXPLAIN = 0.05)
{
  ...
  subclonal.mutations <- which(pyClone.tsv$absolute.ccf.0.95 < 1 &
                                 pyClone.tsv$mutCopyNum > 0.01 &
                                 pyClone.tsv$obs.VAF >= minVAFtoEXPLAIN)
...
calculate_phylo_ccf_withBH <- function(region
  ...
  subclonal.mutations <- which(absolute.ccfs$prob.subclonal > 0.5 &
                                 pyClone.tsv$mutCopyNum > 0.01 &
                                 pyClone.tsv$obs.VAF >= minVAFtoEXPLAIN)
  pyClone.tsv$whichFrac <- NA
  pyClone.tsv$CPNChange <- 0
