...
grow.multi.trees <- function(nestedlist, graph_pyclone, pyclone, ccf_buffer = 10, n_clusters_to_move = 5)
{
  ...
  for (cluster in clusters_with_potential_to_move)
  {
    ...
    for (cluster1 in nesting.to.remove)
    {
      ...
      if (level.issue %in% FALSE &
        circles_exist %in% FALSE &
        new.tree.change)
      {
...
)
{
  ...
  if (is.na(cluster) |
    is.na(clonal_cluster) |
    cluster == clonal_cluster |
    length(clusterMuts) == 1) return(c(p_value = NA,
                                       estimate = NA,
                        ...
                        , force_trunk = TRUE)
{
  ...
  for (clust in clusters_with_multi_input)
  {
    ...
    if (TRUE %in%
      paste(observed_nesting[, 1], observed_nesting[, 2], sep = ":") %in%
      c(paste(strict_nesting[, 1], strict_nesting[, 2], sep = ":")))
    {
  ...
  while ((level_issue | circle) & comb < min(10000, length(muts_to_remove)))
  {
    ...
    if (nrow(toTest) > 1)
    {
      for (row in 1:nrow(toTest))
      {
        ...
        if (test_out$circle %in% FALSE &
          test_out$levelissue %in% FALSE &
          test_out$consistencyissue %in% FALSE)
        {
          level_issue <- FALSE
  ...
  for (cluster in new_clusters_to_test)
  {
    ...
    if (ccf.test$levelissue %in% TRUE | ccf.test$circle %in% TRUE)
    {
      prune.out <- prune.cycle(edgelist = directedGraph_input_pruned_unfolded
        ...
        , max_per_level = max_per_level
        , trunk_cluster = trunk_cluster)
      if (prune.out$levelissue %in% FALSE &
        prune.out$circle %in% FALSE &
        prune.out$consistencyissue %in% FALSE)
      {
                            ...
                            , lower_min = 0)
{
  ...
  for (col in 1:ncol(ccf_ci_boot_lower))
  {
    for (cl in rownames(ccf_ci_boot_lower))
    {
      ...
      if (b$t0 == 1 & length(unique(b$data)) == 1 |
        b$t0 == 1 & length(unique(b$t)) == 1 |
        b$t0 == 0)
      {
        if (b$t0 == 0)
        {
